...
function TicketsPanel({ token }: { token: string }) {
  ...
  return (
    <div>
      <h2 className="mb-6 text-xl font-bold text-gray-900">Bilet Yönetimi</h2>

      {/* QR Verify */}
      <div className="mb-6 rounded-2xl bg-white p-6 shadow-sm border border-gray-100">
        <h3 className="mb-4 font-bold text-gray-800">QR Kod Doğrulama</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center text-gray-400">Bilet bulunamadı</td>
                  </tr>
                ) : (
                  filtered.map((t) => (
                    <tr key={t.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3">
                        <p className="font-medium text-gray-900">{t.user_name || t.guest_email || '—'}</p>
                        {t.user_email && <p className="text-xs text-gray-400">{t.user_email}</p>}
                      </td>
                      <td className="px-4 py-3 text-gray-700">{t.event_title}</td>
                      <td className="px-4 py-3 font-mono text-xs text-gray-400">{t.qr_token.slice(0, 12)}...</td>
                      <td className="px-4 py-3">
                        <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${t.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                          {t.status === 'active' ? 'Aktif' : 'Kullanılmış'}
                        </span>
                      </td>
