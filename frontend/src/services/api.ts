...
const getApiUrl = () => {
  const backendUrl = process.env.EXPO_PUBLIC_BACKEND_URL || 'https://refreshing-determination-production.up.railway.app';
...
class ApiService {
  ...
  async validateDiscount(code: string, eventId: string, quantity: number) {
    const response = await this.axiosInstance.post('/discounts/validate', {
      code,
      event_id: eventId,
      quantity,
    });
...
async getAdminPosts() {
  ...
  async initIyzicoPayment(data: {
    ...
    const response = await this.axiosInstance.post('/payment/iyzico-init', data);
    return response.data as { pending_id: string; payment_url: string };
  }

  async completePayment(pendingId: string) {
    const response = await this.axiosInstance.post(`/payment/complete/${pendingId}`);
  ...
  async getPaymentStatus(pendingId: string) {
    const response = await this.axiosInstance.get(`/payment/status/${pendingId}`);
    return response.data as {
      success: boolean;
      status: string;
      ticket_id?: string;
