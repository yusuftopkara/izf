...
export default function ChallengesScreen() {
  ...
  const completeChallenge = async (challengeId: string) => {
    if (!user) {
      Alert.alert(
        t('challenges.loginRequired'),
        t('challenges.loginPrompt'),
        [
          { text: t('challenges.cancel'), style: 'cancel' },
          { text: t('challenges.login'), onPress: () => router.push('/(auth)/login') },
        ]
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        ...
        t('challenges.completedMessage', { points: result.points, streak: result.new_streak })
      );
      await fetchChallenges();
      await refreshUser();
    } catch (error: any) {
      Alert.alert(t('challenges.error'), error.response?.data?.detail || t('challenges.completeError'));
    } finally {
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.headerRow}>
          <Text style={styles.headerTitle}>{t('nav.challenge')}</Text>
          ...
          >
            <View style={styles.streakContent}>
              ...
              <View style={styles.streakInfo}>
                ...
                <Text style={styles.streakLabel}>{t('challenges.streakDays')}</Text>
              </View>
            </View>
            <View style={styles.streakStats}>
              <View style={styles.streakStat}>
                <Text style={styles.streakStatNumber}>{completedCount}/{challenges.length}</Text>
                <Text style={styles.streakStatLabel}>{t('challenges.completed')}</Text>
              </View>
              <View style={styles.streakDivider} />
              <View style={styles.streakStat}>
                <Text style={styles.streakStatNumber}>{totalPoints}</Text>
                <Text style={styles.streakStatLabel}>{t('challenges.points')}</Text>
              </View>
            </View>
          </LinearGradient>
        </View>
      ...
      >
        <Text style={styles.sectionTitle}>{t('challenges.todayTasks')}</Text>
          ...
          <View key={challenge.id} style={styles.challengeCard}>
            <View style={styles.challengeHeader}>
              ...
              <View style={styles.challengePoints}>
                ...
                <Text style={styles.pointsLabel}>{t('challenges.pointsShort')}</Text>
              </View>
            </View>
                ...
                {completingId === challenge.id ? (
                  ...
                  <>
                    ...
                    <Text style={styles.completeButtonText}>
                      {user ? t('challenges.complete') : t('challenges.loginAndComplete')}
                    </Text>
                  </>
                )}
              </TouchableOpacity>
            ...
            {challenge.completed && (
              <View style={styles.completedBadge}>
                ...
                <Text style={styles.completedText}>{t('challenges.completedBadge')}</Text>
              </View>
            )}
          </View>
        ))}
