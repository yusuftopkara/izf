...
export default function EventDetailScreen() {
  ...
  const fetchEvent = useCallback(async () => {
    try {
      ...
      Alert.alert(t('auth.error'), t('auth.loadFailed'));
    } finally {
      setIsLoading(false);
    }
  }, [id]);
  ...
  const handleAddComment = () => {
    if (!user) {
      Alert.alert(t('auth.loginRequired'), t('auth.commentLoginPrompt'), [
  ...
  const formatDate = (dateStr: string) => {
    try {
      return format(new Date(dateStr), 'd MMMM yyyy, EEEE HH:mm', { locale: locale === 'tr' ? tr : enUS });
    } catch {
      return dateStr;
    }
  };
        ...
        <View style={styles.eventInfo}>
          ...
          <View style={styles.capacityCard}>
            <View style={styles.capacityInfo}>
              ...
              <View style={styles.capacityTextContainer}>
                ...
                <Text style={styles.capacityLabel}>/ {event.capacity} {t('ticket.seatsAvailable')}</Text>
              </View>
            </View>
          ...
          {availableTickets > 0 && (
            <View style={styles.quantitySection}>
              <Text style={styles.quantityLabel}>{t('ticket.quantity')}</Text>
              ...
              <View style={styles.commentForm}>
                ...
                >
                  {isCommenting ? (
                    ...
                    <Text style={styles.submitCommentText}>{t('auth.submit')}</Text>
                  )}
                </TouchableOpacity>
              </View>
            )}
          ...
          >
            {isBuying ? (
              ...
              <Text style={styles.buyButtonText}>
                {availableTickets === 0 ? t('ticket.soldOut') : t('ticket.buy')}
              </Text>
            )}
          </LinearGradient>
        </TouchableOpacity>
      ...
      {event && (
        <PaymentModal
          ...
          event={{
            ...
            discounted_payment_link: event.discounted_payment_link,
          }}
          quantity={quantity}
          user={user ? {
            id: user.id,
